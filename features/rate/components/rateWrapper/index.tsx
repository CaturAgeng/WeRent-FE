...
import FileInput from '../fileInput';
import Description from '../description';
import { dummyUserId, dummyProductId, dummyReview, dummyFitScale, dummyRatingValue, dummyResponseMessage, dummyFile } from '@/app/lib/rating-data'; // Import dummy data

const RateWrapper: React.FC = () => {
    // Inisialisasi state tanpa data dummy
    const [review, setReview] = useState<string>('');
    const [fitScale, setFitScale] = useState<string>('');
    const [file, setFile] = useState<File | null>(null);
    const [ratingValue, setRatingValue] = useState<number>(0);
    ...
    const handleReviewSubmit = () => {
        ...
        // Simulasi pengiriman data dengan dummy response
        console.log("Review submitted with the following data:");
        console.log("User ID:", dummyUserId);
        console.log("Product ID:", dummyProductId);
        console.log("Review:", review || dummyReview);
        console.log("Fit Scale:", fitScale || dummyFitScale);
        console.log("Rating Value:", ratingValue || dummyRatingValue);
        console.log("File:", file || dummyFile);

        setResponseMessage(dummyResponseMessage);
        console.log("Review submitted successfully with dummy data");
        setShowErrors(false);
        setReview('');
        setFitScale('');
        setFile(null);
    ...
    return (
        <div className="p-4 max-w-md mx-auto bg-white rounded-xl shadow-md space-y-4">
            <div className="flex items-center">
                <div>
                    <StarRating userId={dummyUserId} productId={dummyProductId} value={ratingValue} onChange={setRatingValue} required={showErrors} showErrors={showErrors} />
                    <FitScale fitScale={fitScale} setFitScale={setFitScale} required={showErrors} />
                    {showErrors && (!fitScale || ratingValue === 0) && <p className='text-red-600 text-sm mt-2'>Fit scale is required.</p>}
                </div>
            </div>
